...
async def handle_websocket(websocket, path):
    async for message in websocket:
        ...
        if buffer_data is not None:
            ...
            scaled_data = scaled_data.reshape(-1, 30, 12, 1)
            # print(scaled_data)
            model = keras.models.load_model("/root/GT/Hacklytics/hand-tracking/Websockets/pretrained")
            prediction = model.predict(scaled_data)
            print("Model prediction", np.argmax(prediction, axis=1))
